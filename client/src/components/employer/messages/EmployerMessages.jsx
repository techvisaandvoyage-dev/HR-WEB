...
const EmployerMessages = ({ candidates = [], triggerNavRefresh, updateCandidateStatus }) => {
  ...
  const fetchConversations = async () => {
    try {
      const res = await fetch('https://hr-website-kzdw.onrender.com/api/employer/messages/conversations', {
  ...
  const fetchMessages = async (appId) => {
    try {
      const res = await fetch(`https://hr-website-kzdw.onrender.com/api/employer/messages/applications/${appId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('employerToken')}` }
      });
      const data = await res.json();
      if (data.success) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(`https://hr-website-kzdw.onrender.com/api/employer/messages/applications/${selectedApplication.appId}`, {
        method: 'POST',
        headers: {
