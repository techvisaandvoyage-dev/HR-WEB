...
const EmployerSettingsTab = ({ portalConfig }) => {
  ...
  const fetchSecurityStatus = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/employer/auth/security`, {
        headers: {
  ...
  // Handle Submit: Set Password (For Google / Passwordless Users)
  const handleSetPasswordSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/employer/auth/set-password`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok && data.success) {
        ...
        showToast('Password created successfully! You can now log in using your email and password.');
      } else {
        setSetForm(prev => ({
          ...prev,
  ...
  // Step 1 Submit: Request OTP for Changing Password
  const handleRequestChangePasswordOtp = async (e) => {
    e.preventDefault();
    setChangeForm(prev => ({ ...prev, error: '' }));
    ...
    if (!criteria.isValid) {
      setChangeForm(prev => ({ ...prev, error: 'New password does not fulfill security requirements.' }));
      return;
    }
    ...
    if (changeForm.currentPassword === changeForm.newPassword) {
      setChangeForm(prev => ({ ...prev, error: 'New password must be different from current password.' }));
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/employer/auth/change-password/send-otp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (res.ok && data.success) {
        ...
        setChangeForm(prev => ({
          ...
          submitting: false,
          error: '',
          successInfo: data.message || `Verification code sent to ${maskEmail(securityData.email)}`
        }));
      } else {
        setChangeForm(prev => ({
          ...prev,
          error: data.message || 'Failed to verify current password.',
          submitting: false
        }));
      }
    } catch (err) {
  ...
  const handleResendChangePasswordOtp = async () => {
    ...
    setChangeForm(prev => ({ ...prev, resending: true, error: '' }));
    const token = localStorage.getItem('employerToken');

    try {
      const res = await fetch(`${API_BASE}/api/employer/auth/change-password/resend-otp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (res.ok && data.success) {
        ...
        setChangeForm(prev => ({
          ...prev,
          resending: false,
          successInfo: data.message || 'A new verification code has been sent to your email.'
        }));
      } else {
        setChangeForm(prev => ({
          ...prev,
      ...
      setChangeForm(prev => ({
        ...prev,
        resending: false,
        error: 'Network error while resending OTP.'
      }));
    }
  };

  // Step 2 Submit: Verify OTP and Finalize Password Update
  const handleVerifyAndChangePassword = async (e) => {
    e.preventDefault();
    setChangeForm(prev => ({ ...prev, error: '' }));

    if (!changeForm.otp || changeForm.otp.trim().length < 4) {
      setChangeForm(prev => ({ ...prev, error: 'Please enter the valid OTP code.' }));
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/employer/auth/change-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const displayEmail = securityData.email || 'your email';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-10">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-[26px] font-bold text-[#147a2e] tracking-tight uppercase">Settings &amp; Security</h1>
          <p className="text-gray-500 text-sm mt-1">Manage your account authentication credentials and security options.</p>
        </div>
      </div>
            ...
            <div className="flex-shrink-0">
              {!securityData.hasPassword ? (
                ...
                <button
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-[#29953f] hover:bg-green-700 text-white font-bold text-sm rounded-xl shadow-xs transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-[#29953f] active:scale-[0.98]"
                >
                ...
                <form onSubmit={handleRequestChangePasswordOtp} className="mt-5 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-3.5 py-2.5 pr-11 bg-white border border-gray-300 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#29953f] focus:border-[#29953f] transition-all"
                        required
                      />
                      <button
                        type="button"
                  ...
                  <div className="pt-3 flex items-center justify-end gap-3 border-t border-gray-100 mt-6">
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-[#29953f] hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-[#29953f]"
                    >
                ...
                <form onSubmit={handleVerifyAndChangePassword} className="mt-5 space-y-4">
                  ...
                  <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl text-center space-y-3">
                    ...
                    <input
                      ...
                      placeholder="• • • •"
                      className="w-44 mx-auto px-4 py-3 bg-white border-2 border-emerald-400 rounded-xl text-center font-mono font-bold text-xl tracking-[0.3em] text-gray-900 focus:outline-none focus:ring-4 focus:ring-emerald-100 transition-all shadow-inner"
                      required
                    />
                  ...
                  <div className="flex items-center justify-between text-xs px-1">
                    ...
                    {changeCooldown > 0 ? (
                      ...
                      <button
                        ...
                        className="text-[#29953f] hover:text-green-700 font-bold hover:underline disabled:opacity-50"
                      >
...
export default EmployerSettingsTab;
