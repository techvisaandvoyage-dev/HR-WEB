...
const EmployeeMessages = () => {
  ...
  const fetchConversations = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/employee/messages/conversations', {
  ...
  const fetchMessages = async (applicationId) => {
    try {
      const res = await fetch(`http://localhost:5000/api/employee/messages/applications/${applicationId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('employeeToken')}` }
      });
      const data = await res.json();
      if (data.success) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/employee/messages/applications/${selectedChat.applicationId}`, {
        method: 'POST',
        headers: {
